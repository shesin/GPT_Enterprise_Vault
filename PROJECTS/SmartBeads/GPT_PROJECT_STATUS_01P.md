...
- **Jest (2026-08-20):** Production PvE sanity is `FeatureSession` + `HonestAi` + `runAiTurn` on **all seven V1 boards** (`v1ProductionSanity.test.ts`). Lab `.cjs` N=100 and Board4 `SelfPlayRunner` are not that path. Technical verification only — not UI proof.
