...
- **Play shell themes (2026-09-14):** Lovable OKLCH **Look preview** — **two rows:** **Dark theme** (4 dark boards + charcoal sides) · **Light theme** (Sandy Beige · Soft Blush · Pale Sage · Warm Walnut + charcoal sides). Hub **Choose your look** (`#hub-play-theme-setting`) before play; board settings mirror (`#play-theme-setting`, locked after first move). Row 3 (matched dark sides) **removed**; old matched storage migrates to dark theme + charcoal. Storage `sb-play-board-look` + `sb-play-side-look-v3` source of truth; `syncPlayLookFromStorageIfDrifted` repairs drift; `syncThemeSwatchActive` clears stale `is-active` before setting one swatch. **Board grid lines:** `CanvasBoardRenderer` uses locked **Classic gold** from `boardLineGoldThemes` (not per-theme OKLCH `lineColor` — fixes vanishing lines on Warm Walnut / light boards). **TESTED** (Jest + Playwright incl. `m2-walnut-130s-verify.mjs` 130s). **Human browser UNCONFIRMED**.
...
- **Move hint aura (2026-09-13, renamed 2026-09-21):** Board settings `#move-hint-aura-select` — **Off** (default) · **Original (orange / lime)** · **Gold** · **Black Gold** (darker gold for light boards where plain Gold lacks contrast); each = snug centre glow + bright rim hugging bead. **TESTED** (`moveHintAuraThemes`, `CanvasBoardRenderer.moveFeedback`). **Human browser UNCONFIRMED**.
...
| **Recent colour / panel edits** | **Partial (2026-09-14)** — 6 complete OKLCH looks + charcoal side-only; bead set picker; board look persistence fix. **UNCONFIRMED** full browser sign-off |
| **Play shell themes (board settings)** | **OK (Jest, 2026-09-14)** — 6 unified complete + side-only charcoal; board canvas from stored board id. **UNCONFIRMED** human browser |
| **Move hint aura** | **OK (Jest, 2026-09-13, updated 2026-09-21)** — Off / Original / Gold / Black Gold. **UNCONFIRMED** human browser |
