...
const FILE = path.join(__dirname, 'SHOLO_GUTI_3_BEAD_WITH_FEATURE.html');
const ENGINE = './sholo-3-bead-3x5-fullturn-engine.cjs';
...
const api = sandbox.window.__SHOLO_GUTI_3_FEATURE__;
assert(api, 'API missing');
assert(api.N === 15, 'expected 15 nodes');
const b = api.getBoard();
assert(b.filter((x) => x === api.P1).length === 3, 'P1 count');
assert(b.filter((x) => x === api.P2).length === 3, 'P2 count');
assert(api.NODES.filter((n) => n.y === 0).every((n) => b[api.NODE_INDEX[n.id]] === api.P2), 'row 1 P2');
assert(api.NODES.filter((n) => n.y === 8).every((n) => b[api.NODE_INDEX[n.id]] === api.P1), 'row 5 P1');
assert(api.NODES.filter((n) => n.y === 4).every((n) => b[api.NODE_INDEX[n.id]] === 0), 'row 3 empty');
const engine = require(ENGINE);
assert(engine.startingBoard().join('') === b.join(''), 'engine parity');
const out = { file: path.basename(FILE), engine: ENGINE, ok: true };
fs.writeFileSync(path.join(__dirname, 'SHOLO_3_BEAD_3X5_FEATURE_SMOKE.json'), JSON.stringify(out, null, 2));
