...
1. **`16`'s D2 fairness-gate failure and low resolve rate (added 2026-09-23)** — the only finding in this review that looks like a real candidate for VISION's "genuine Lab or gameplay failure" threshold. Needs a human call: re-run with a larger/cleaner sample to confirm, investigate root cause (AI struggling on the largest board's search space, or a genuine geometry asymmetry), or treat as accepted since it's the long-shipped "standard" board. Not acted on.
