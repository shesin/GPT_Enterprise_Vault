...
export class CoachVideoPlayer {
  ...
  private fireCues(prev: number, next: number): void {
    ...
    for (const cue of cues) {
      if (cue.atMs > prev && cue.atMs <= next && !this.firedCues.has(cue.atMs)) {
  ...
  private applyCueAtSeek(ms: number): void {
    this.callbacks.onApplyCue(findCoachCueAtTime(ms, this.script.cues ?? []));
  }
  ...
  private syncFiredSets(): void {
    ...
    this.firedCues = new Set(
      (this.script.cues ?? []).filter((c) => c.atMs <= this.timeMs).map((c) => c.atMs),
    );
