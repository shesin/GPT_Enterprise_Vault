...
const FILE = path.join(__dirname, 'SHOLO_GUTI_5_BEAD_WITH_FEATURE.html');
...
assert(/5×3|5 x 3|COLS = 5/.test(html), '5-bead should document 5×3 / COLS=5');
assert(/isEndgameCenterNode/.test(html), 'center highlight helper missing');
assert(/__SHOLO_GUTI_5_FEATURE__/.test(html), 'API name missing');
...
const api = sandbox.window.__SHOLO_GUTI_5_FEATURE__;
...
assert(p1 === 5 && p2 === 5, 'start not 5/5: ' + p1 + '/' + p2);

const midRow = api.NODES.filter((n) => n.y === 2);
assert(midRow.length === 5, 'middle row should have 5 nodes');
assert(midRow.every((n) => b[api.NODE_INDEX[n.id]] === 0), 'middle row must start completely empty');

const centerNodes = api.NODES.filter((n) => n.y === 2 && n.x === 4);
assert(centerNodes.length === 1, 'centre zone should be single node at middle row col 3');

const topRow = api.NODES.filter((n) => n.y === 0);
const bottomRow = api.NODES.filter((n) => n.y === 4);
assert(topRow.every((n) => b[api.NODE_INDEX[n.id]] === api.P2), 'top row must be all P2 (Ebony)');
assert(bottomRow.every((n) => b[api.NODE_INDEX[n.id]] === api.P1), 'bottom row must be all P1 (Ivory)');

assert(api.getAllLegalMoves().length > 0, 'no opening moves');

const out = {
  file: 'SHOLO_GUTI_5_BEAD_WITH_FEATURE.html',
  N: api.N,
  start: { p1, p2 },
...
fs.writeFileSync(path.join(__dirname, 'SHOLO_5_BEAD_FEATURE_SMOKE.json'), JSON.stringify(out, null, 2));
