...
1. **M3 — port remaining locked product boards** (5 of 7 in production: 16, 6×4, 6×3×5, 10×5, 12×6×5).  
...
- **M0 + M1 (2026-08-18):** Production **16-bead Sholo Guti** — `Board16Sholo.ts`, `BoardCatalog.ts`, `terminationProfile: sholo_guti`. **76 Jest tests pass** (2026-08-19).
- **M2 + M3 shell (2026-08-19):** Shared play shell — `PlayController.ts`, `FeatureSession.ts`, `CanvasBoardRenderer.ts`, board `<select>`, centre rules, undo, honest AI. Browser scripts: `m2-browser-verify.mjs`, `m2-6x4-browser-verify.mjs`, `m2-6x3x5-browser-verify.mjs`, `m2-10x5-browser-verify.mjs`, `m2-12x6x5-browser-verify.mjs`.
...
1. **M3 — next locked board:** 8-bead · 4×6 hourglass (`8x4x6`) — new `BoardDefinition` + catalog enable.  
