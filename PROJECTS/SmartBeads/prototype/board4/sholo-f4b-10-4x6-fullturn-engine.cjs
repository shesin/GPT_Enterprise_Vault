'use strict';
/**
 * Headless 7-bead / 4×5 Lab engine.
 * Geometry + start match SHOLO_GUTI_7_BEAD_WITH_FEATURE.html (column layout 5+2+2+5).
...
function startingBoard() {
  ...
  for (let i = 0; i < N; i++) {
    ...
    else if (p.x === 2 && (p.y === 0 || p.y === 2 || p.y === 8 || p.y === 10)) b[i] = P1;
    else if (p.x === 4 && (p.y === 0 || p.y === 2 || p.y === 8 || p.y === 10)) b[i] = P2;
  }
  return b;
}
