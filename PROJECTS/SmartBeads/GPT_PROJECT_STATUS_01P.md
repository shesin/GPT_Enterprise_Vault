...
- **16-bead wing geometry & visual proportions (2026-08-24):** Corrected triangular wing lattice coordinates (`x=-4,-2` left, `x=10,12` right) and updated canvas layout to 480×872 (`SHOLO16_CANVAS`). The central 5×5 playing grid is rendered as a prominent, uncompressed 400×400px square (two-thirds of total board area) with compact triangular wings (200px height). Diagonals crossing the apex are perfectly collinear at exact 45° angles. Verified across all 369 Jest tests and live Playwright browser gates.
