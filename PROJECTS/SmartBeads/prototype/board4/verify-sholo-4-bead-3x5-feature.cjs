...
const FILE = path.join(__dirname, 'SHOLO_GUTI_4_BEAD_WITH_FEATURE.html');
const ENGINE = './sholo-4-bead-3x5-fullturn-engine.cjs';
...
const api = sandbox.window.__SHOLO_GUTI_4_FEATURE__;
assert(api, 'API missing');
const b = api.getBoard();
assert(b.filter((x) => x === api.P1).length === 4, 'P1 count');
assert(b.filter((x) => x === api.P2).length === 4, 'P2 count');
const outer = (n) => (n.y === 0 || n.y === 2 || n.y === 6 || n.y === 8) && n.x !== 2;
assert(api.NODES.filter((n) => outer(n) && n.y <= 2).every((n) => b[api.NODE_INDEX[n.id]] === api.P2), 'top outer');
assert(api.NODES.filter((n) => outer(n) && n.y >= 6).every((n) => b[api.NODE_INDEX[n.id]] === api.P1), 'bottom outer');
const engine = require(ENGINE);
assert(engine.startingBoard().join('') === b.join(''), 'engine parity');
const out = { file: path.basename(FILE), engine: ENGINE, ok: true };
fs.writeFileSync(path.join(__dirname, 'SHOLO_4_BEAD_3X5_FEATURE_SMOKE.json'), JSON.stringify(out, null, 2));
