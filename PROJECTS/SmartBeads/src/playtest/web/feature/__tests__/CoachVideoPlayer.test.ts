...
import { COACH_VIDEO, COACH_VIDEO_WIN_SEGMENT_START_MS, type CoachVideoCue, type CoachVideoHighlight, type CoachVideoKeyframe, type CoachVideoMove, type CoachVideoSegmentBanner, type CoachVideoSpeech } from '../CoachVideoScript';
...
describe('CoachVideoPlayer', () => {
  ...
  it('applies scripted cues when seeking into ending segments', () => {
    ...
    player.seek(COACH_VIDEO_WIN_SEGMENT_START_MS + 5_000);
    expect(cues.at(-1)?.kind).toBe('result');

    player.seek(COACH_VIDEO_WIN_SEGMENT_START_MS + 7_000);
