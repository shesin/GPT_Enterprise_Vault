/**
 * Browser verification for 10-bead · 5×5 (catalog id 10x5).
...
async function main() {
  ...
  try {
    ...
    record('board selector lists 10x5', boardOptions.some((t) => /10-bead.*5×5|10-bead.*5x5/i.test(t)), boardOptions.join(' | '));

    await selectBoard(page);

    const p1 = await page.locator('#p1-pieces').textContent();
    const p2 = await page.locator('#p2-pieces').textContent();
    record('starting pieces 10+10', p1 === '10' && p2 === '10', `P1=${p1} P2=${p2}`);
    ...
    record(
      'geometry 25 nodes / 72 edges / centre 12',
      geom.nodes === 25 && geom.edges === 72 && geom.centers?.[0] === 12,
      JSON.stringify(geom),
    );
    record('empty centre file (5 nodes)', geom.emptyIds.join(',') === '2,7,12,17,22', geom.emptyIds.join(','));
