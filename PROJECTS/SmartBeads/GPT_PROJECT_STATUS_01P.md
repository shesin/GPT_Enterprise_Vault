...
- **Play shell themes (2026-09-14):** Lovable OKLCH look swatches on **board settings** (`#play-theme-setting`) — **Complete (board + side)** ×6 (Classic Green · Wood Classic · Ocean Blue · Sandy Beige · Forest Green · Purple Night; one colour family throughout) and **Board stays same — side panels only** (charcoal id `7`). Storage `sb-play-board-look` + `sb-play-side-look-v3`; board id preserved when side-only picked. Bead set picker (`#bead-set-select`). **TESTED** (`playShellThemes`, `boardLookThemes`, `hubShell`, `processRegressionGuards`). **Human browser UNCONFIRMED**.
