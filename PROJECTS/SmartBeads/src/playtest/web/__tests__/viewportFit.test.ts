...
const css = fs.readFileSync(path.resolve(__dirname, '../play-shell.css'), 'utf8');
...
describe('viewport fit CSS contract', () => {
  it('sizes board on .shell with height-first and horizontal budget', () => {
    ...
    expect(css).toContain(
      '--board-area-w: max(160px, calc(100vw - var(--side-columns-width) - var(--shell-gutter-h)))',
    );
    expect(css).toContain('--frame-h: min(');
    expect(css).toContain('calc(var(--board-area-w) / var(--board-aspect))');
    expect(css).toContain(
      '--frame-w: min(var(--board-area-w), calc(var(--frame-h) * var(--board-aspect)))',
    );
