...
- **Play shell themes (2026-09-13):** Lovable-aligned look swatches on **board settings** only (`#play-theme-setting`) — two groups: **Complete (board + side)** (teal · plum · forest & gold · warm brown) and **Board stays same — side panels only** (charcoal + gold). Separate storage `sb-play-board-look` / `sb-play-side-look`; defaults forest & gold board + charcoal side. Warm parchment retired. Classic gold grid lines locked. **TESTED** (`playShellThemes`, `hubShell`, `processRegressionGuards`). **Human browser UNCONFIRMED**.
- **Move hint aura (2026-09-13):** Board settings `#move-hint-aura-select` — **Off** (default) · **Original (orange / lime)** · **Gold (fill)**; gold fill = centre glow + twin rings flush on bead edge. **TESTED** (`moveHintAuraThemes`, `CanvasBoardRenderer.moveFeedback`). **Human browser UNCONFIRMED**.
...
| **Recent colour / panel edits** | **Partial (2026-09-13)** — Lovable look swatches; move hint Off/Original/Gold-fill; SFX default off; BGM no pre-selected track. **UNCONFIRMED** full browser sign-off |
| **Play shell themes (board settings)** | **OK (Jest, 2026-09-13)** — complete vs side-only split; board canvas independent when side-only swatch picked. **UNCONFIRMED** human browser |
