'use strict';
/**
 * Lab-compare 7-bead (4×5) candidate vs 16-bead standard under the same protocol.
 */
const fs = require('fs');
const path = require('path');
const vm = require('vm');
const eng16 = require('./sholo-guti-fullturn-engine.cjs');
const engSlice = require('./sholo-7-bead-fullturn-engine.cjs');
...
const SLICE = 7;
const HTML_SLICE = path.join(__dirname, 'SHOLO_GUTI_7_BEAD_WITH_FEATURE.html');
const OUT = path.join(__dirname, 'SHOLO_7_VS_16_LAB_COMPARE.json');
const API_NAME = '__SHOLO_GUTI_7_FEATURE__';
...
function main() {
  ...
  const report = {
    purpose: 'Compare 7-bead/4×5 candidate vs 16-bead standard — metrics and evidence only (no board verdict)',
    authoritativeEvaluator: 'evaluate-ladder-lab.cjs',
    candidateFile: 'SHOLO_GUTI_7_BEAD_WITH_FEATURE.html',
    candidateEngine: 'sholo-7-bead-fullturn-engine.cjs',
