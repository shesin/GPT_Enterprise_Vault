...
- **Play shell themes (2026-09-14):** Lovable OKLCH **Look preview** on board settings (`#play-theme-setting`) — three rows: **Dark board — charcol side panel** (4 dark + charcoal sides) · **Light board — charcoal side panel** (Sandy Beige · Soft Blush · Pale Sage · Warm Walnut) · **Dark board — same side panel** (4 matched dark). Row-aware clicks (`applyPlayLookFromRow`); board frame uses token `frameOuter`/`frameInner`. Storage `sb-play-board-look` + `sb-play-side-look-v3` is source of truth (`readBoardLookThemeId` / `readSideLookThemeId`); `syncPlayLookFromStorageIfDrifted` repairs DOM/CSS drift on draw/UI tick. Bead set picker (`#bead-set-select`). **TESTED** (Jest + Playwright verify/drift/**130s gameplay** scripts). **Human browser UNCONFIRMED**.
...
- **Move hint aura (2026-09-13):** Board settings `#move-hint-aura-select` — **Off** (default) · **Original (orange / lime)** · **Gold (fill)**; gold fill = snug centre glow + bright rim hugging bead. **TESTED** (`moveHintAuraThemes`, `CanvasBoardRenderer.moveFeedback`). **Human browser UNCONFIRMED**.
...
| **Move hint aura** | **OK (Jest, 2026-09-13)** — Off / Original / Gold (fill). **UNCONFIRMED** human browser |
