...
const refEngine = requireRef(
  path.resolve(process.cwd(), 'PROJECTS/SmartBeads/prototype/board4/sholo-7-bead-fullturn-engine.cjs'),
);

describe('Board7 prototype parity', () => {
  it('matches headless reference node, edge, and jump counts', () => {
    expect(BOARD7_NODE_COUNT).toBe(refEngine.N);
    expect(BOARD7_EDGE_COUNT).toBe(countRefEdges());
    expect(BOARD7_JUMP_COUNT).toBe(countRefJumps());
  });
  ...
  it('matches reference start fingerprint', () => {
    ...
    const prodStart = Board7.intersections
      .map((point) => {
    ...
    expect(prodStart).toBe('11221002100210021122');
  });

  it('matches reference collinear capture routes on sample adjacency', () => {
    const byLabel = new Map(Board7.intersections.map((point) => [point.label!, point.id]));
